...
class GrammarChecker {
     ...
     */    animatePageLoad() {
        ...
        // MacOS-like reveal animation timeline
        const timeline = anime.timeline({
            easing: 'easeOutExpo'
        });

        // Animate header with subtle slide and fade
        timeline.add({
            targets: '.header',
            translateY: [-20, 0],
            opacity: [0, 1],
        ...
        // Animate panels with scale and fade effect
        timeline.add({
        ...
        // Animate metric cards with elastic effect
        timeline.add({
            targets: '.metric-card',
            scale: [0.85, 1],
            opacity: [0, 1],
        ...
        // Animate buttons with subtle bounce
        timeline.add({
            targets: '.btn',
            translateY: [15, 0],
            opacity: [0, 1],
        ...
        // Animate any nav buttons
        timeline.add({
            targets: '.nav-btn, .nav-btn-primary',
            translateY: [10, 0],
            opacity: [0, 1],
...
You can upload documents in multiple formats, get synonym suggestions, and export detailed reports. Weather you're writing academic papers, business emails, or creative content, OpenGrammar provides the insights you need to enhance your writing quality.`;
     ...
     */    initializeLoadingScreen() {
        console.log('initializeLoadingScreen called');
        ...
        console.log('Loading state initialized:', this.loadingState);
        ...
        setTimeout(() => {
            console.log('Components ready, checking if loading complete');
    ...
    addLoadingScreenKeyHandler() {
        ...
        const skipButton = document.getElementById('skipLoadingBtn');
        if (skipButton) {
        ...
        // Don't auto-remove the handler until loading screen is hidden
        // This ensures the 'S' key works throughout the loading process
    }    /**
     ...
     */    hideLoadingScreen() {
        ...
        // Remove keyboard handler when hiding loading screen
        document.removeEventListener('keydown', this.skipHandler);

        if (typeof anime !== 'undefined') {
            // First animate the loading content with macOS-like zoom out effect
            ...
            }, '-=400');  // Start slightly before the first animation completes
        } else {
            // Fallback without anime.js
            const content = document.querySelector('.loading-content');
            content.style.transition = 'opacity 0.6s ease-out, transform 0.6s ease-out, filter 0.6s ease-out';
     ...
     */    animateLoadingProgress() {
        ...
        if (typeof anime !== 'undefined') {
            // Animate progress bar with macOS-like stages
            anime.timeline({
                easing: 'easeOutExpo'
            })
            .add({
     ...
     */    showRandomLoadingTips() {
        const tips = [
            ...
            'Calibrating writing tools...',
            'Enhancing vocabulary suggestions...',
        ...
        if (typeof anime !== 'undefined') {
            const showNextTip = () => {
                // Only proceed if we haven't shown all tips yet
                ...
                // Only continue cycling through tips for a limited time
                if (currentTipIndex < tips.length - 1 && currentTipIndex < 4) {
                    setTimeout(showNextTip, 2000);
                }
            };
            
            // Start the tip animation cycle
            showNextTip();
        } else {
            // Fallback without anime.js
            const tipInterval = setInterval(() => {
                ...
                tipElement.style.transform = 'translateY(-8px)';
                tipElement.style.transition = 'opacity 0.4s ease, transform 0.4s ease';
                
                setTimeout(() => {
                    tipElement.textContent = tips[currentTipIndex];
                    tipElement.style.opacity = '0.85';
                    tipElement.style.transform = 'translateY(0)';
                }, 400);
                
                // Only show a few tips
     ...
     */    updateLoadingMessage(message) {
        ...
        if (locationWelcome && message) {
            // For a more macOS-like animation
            if (typeof anime !== 'undefined') {
                anime.timeline({
                    easing: 'easeOutExpo'
                })
                ...
                .add({
                    ...
                    borderRadius: '4px',
                    padding: ['2px 4px', '2px 8px', '2px 4px'],
                    duration: 1200,
                    easing: 'easeInOutQuad'
                }, '-=400');
            } else {
                // Fallback animation
                locationWelcome.style.opacity = '0';
                locationWelcome.style.transform = 'translateY(-10px)';
                locationWelcome.style.transition = 'opacity 0.3s ease, transform 0.3s ease';
                
                setTimeout(() => {
                    locationWelcome.textContent = message;
                    locationWelcome.style.opacity = '0.95';
                    locationWelcome.style.transform = 'translateY(0)';
                }, 300);
            }
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
        ...
        console.log('GrammarChecker successfully initialized');
    } catch (error) {
        console.error('Error initializing GrammarChecker:', error);
    }
});
